...
async function migrate() {
    ...
    await set(ref(db, "projects"), projectsObj);
    console.log("Projects migrated.");

    // 2. Migrate Skills
    const skillsObj = {};
    Object.entries(DEFAULT_SKILLS).forEach(([category, items]) => {
        skillsObj[category] = { items };
    });
    await set(ref(db, "skills"), skillsObj);
    console.log("Skills migrated.");

    // 3. Migrate Config
    await set(ref(db, "config/landing"), DEFAULT_CONFIG);
